...
def create_sync_result_embed(
    ...
    if has_failed and not has_added:
        ...
        if len(result.target_roles_calculated or []) == 0:
            if result.mutual_guilds_found == 0:
                # Бот не нашёл пользователя ни на одном фракционном сервере.
                # Обычно значит: пользователь не состоит в сервере фракции,
                # либо бот не добавлен в этот сервер.
                title = "⚠️ Вы не найдены на серверах фракций"
                description = (
                    "Бот не нашёл вас ни на одном Discord-сервере фракции.\n"
                    "Убедитесь, что вы состоите в основном Discord вашей фракции."
                )
            else:
                # Пользователь найден, но ни одна его роль не настроена в маппинге.
                title = "⚠️ Нет подходящих ролей"
                description = (
                    "Вы найдены на сервере фракции, но ни одна из ваших ролей "
                    "не настроена для синхронизации."
                )
            color = COLOR_WARNING
        else:
            title = "ℹ️ Актуально"
