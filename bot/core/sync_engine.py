...
class SyncResult:
    ...
    # True, если хотя бы с одного source-сервера не удалось прочитать данные
    # (транзиентная ошибка). При этом снятие ролей подавляется, а монитор
    # запланирует повторную попытку.
    ...
    async def sync_user_roles(
        ...
        *,
        ...
        try:
            ...
            # Защита от ложного снятия ролей при неполных данных.
            # Если хотя бы с одного source-сервера не удалось прочитать роли
            # (транзиентная ошибка), картина неполная: добавлять роли безопасно
            # (есть позитивное подтверждение), но СНИМАТЬ нельзя — иначе можно
            # снять роль, оправдание которой пришло бы со «сломанного» сервера.
            if fetch_errors and roles_to_remove:
                result.data_incomplete = True
                logger.warning(
                    f"Неполные данные с source-серверов для {user_id} "
                    f"({len(fetch_errors)} ошибок): подавляю снятие "
                    f"{len(roles_to_remove)} ролей, чтобы не снять по ошибке. "
                    f"Будет повторная попытка."
                )
                roles_to_remove = []
            elif fetch_errors:
                # Ошибки чтения есть, но снимать всё равно нечего — помечаем
                # неполноту, чтобы монитор повторил попытку.
                result.data_incomplete = True
